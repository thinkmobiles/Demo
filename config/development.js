'use strict';

process.env.WEB_HOST = 'http://127.0.0.1:8838';
process.env.PORT = 8838;

process.env.DB_NAME = 'rocketDB';
process.env.DB_HOST = '192.168.88.250';//
//process.env.DB_HOST = '127.0.0.1'; //local

process.env.REDIRECT_URI = "http://127.0.0.1:8838/redirect";
process.env.HOME_PAGE = "http://demo.com:8838/#/home/";

process.env.mailerService = 'Gmail';
process.env.mailerUserName = "gogi.gogishvili";
process.env.mailerPassword = "gogi123456789";

process.env.JUMPLEAD_CLIENT_ID = "uemnB2ZAA92gv5CoTCHc";
process.env.JUMPLEAD_CLIENT_SECRET = "cPtrcBklxNUEnVL3VBETsKhsDGQHGF73wkYcwWU9";

process.env.AMAZON_ACCESS_KEY_ID = 'AKIAJPUAJM7ESJZR33OQ';
process.env.AMAZON_SECRET_ACCESS_KEY = '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';